...
export default function ExecutiveReport() {
  ...
  // Query state for server-side operations
  const [queryParams, setQueryParams] = useState({
    page:     1,
    pageSize: 10,
    search:   "",
    sortBy:   "completedCalls",
    sortDir:  "desc",
  });
  ...
  const fetchData = useCallback(async (params) => {
    ...
    setError(null);
    try {
      const result = await salesTLReportsService.getExecutive(params);
      setExecutives(result.executives || []);
      setPagination(result.pagination || DEFAULT_PAGINATION);
    } catch (err) {
      setError(err?.response?.data?.message || err.message || "Failed to load executive report");
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => { fetchData(queryParams); }, [fetchData, queryParams]);
  ...
  const handleSort = (sortBy, sortDir) => {
    setQueryParams((prev) => ({ ...prev, page: 1, sortBy, sortDir }));
  };
      ...
      {error && (
        <DashGrid cols={12} gap={4}>
          <div className="col-span-12">
            <ErrorBanner message={error} onRetry={() => fetchData(queryParams)} />
          </div>
        </DashGrid>
      )}
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-end pt-2 border-t border-slate-100">
              <Button
                ...
                variant="ghost"
                size={3}
                onClick={() => closeModal("tl-exec-report-view")}
              />
            </div>
          </div>
        )}
